  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Users', value: mockUsers.length, icon: UserCheck, color: 'text-cyan-400' },
          { label: 'Active', value: mockUsers.filter(u => u.status === 'active').length, icon: UserCheck, color: 'text-emerald-400' },
          { label: 'Suspended', value: mockUsers.filter(u => u.status === 'suspended').length, icon: UserX, color: 'text-red-400' },
          { label: 'Admins', value: mockUsers.filter(u => u.role === 'admin').length, icon: Shield, color: 'text-violet-400' },
        ].map((stat) => (
            ...
            <tbody>
              {filtered.map((user) => (
                <tr key={user.id} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4 hidden lg:table-cell text-sm text-slate-300 font-medium">{user.trips}</td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button className="p-1.5 rounded-md text-slate-400 hover:text-violet-400 hover:bg-violet-500/10 transition-all" title="Promote">
                        <Shield className="w-4 h-4" />
                      </button>
                      <button className="p-1.5 rounded-md text-slate-400 hover:text-amber-400 hover:bg-amber-500/10 transition-all" title="Suspend">
                        <Ban className="w-4 h-4" />
                      </button>
                      <button className="p-1.5 rounded-md text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-all" title="Delete">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900 rounded-2xl border border-slate-700 w-full max-w-lg p-8 shadow-2xl">
            ...
            <div className="flex gap-3 mt-8">
              <button className="flex-1 py-2.5 rounded-lg bg-violet-500/10 text-violet-400 border border-violet-500/20 text-sm font-semibold hover:bg-violet-500/20 transition-all flex items-center justify-center gap-2">
                <Shield className="w-4 h-4" /> Promote
              </button>
              <button className="flex-1 py-2.5 rounded-lg bg-amber-500/10 text-amber-400 border border-amber-500/20 text-sm font-semibold hover:bg-amber-500/20 transition-all flex items-center justify-center gap-2">
                <Ban className="w-4 h-4" /> Suspend
              </button>
              <button className="flex-1 py-2.5 rounded-lg bg-red-500/10 text-red-400 border border-red-500/20 text-sm font-semibold hover:bg-red-500/20 transition-all flex items-center justify-center gap-2">
